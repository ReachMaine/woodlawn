<!DOCTYPE html>
<? /*mods:
* zig 4March15 - add google tag manager function call for DuracellTomi's Tag manager plugin
* zig 7march15 - only put afterhtml on frontpage.
* zig 4Apr16 -  use  html_intro for the top image (on all pages & after topbar )
* zig 28Apr16 - add font awesome icons
 */ ?>
...
if($flatsome_opt['html_intro'] && is_front_page()) echo '<div class="home-intro">'.do_shortcode($flatsome_opt['html_intro']).'</div>' */ ?>
	...
	<div id="wrapper"<?php if($flatsome_opt['box_shadow']) echo ' class="box-shadow"';?>>
		...
		<?php }?>
		<?php if ($flatsome_opt['html_intro']) echo '<div class="home-intro">'.do_shortcode($flatsome_opt['html_intro']).'</div>' /* zig - moved to here */ ?> 
...
<?php if($flatsome_opt['html_after_header'] && is_front_page() ){
